...
- The app loads organization, cost center, sales channels, marketing tags, B2B settings, and **app settings** (from manifest) in parallel. App settings are cached in memory (LRU, 1 minute) to avoid calling the Apps API on every request.
...
- **TTL:** 5 minute
- **Effect:** Changes to app settings in the Admin can take up to 1 minute to apply to `setProfile` behavior.
