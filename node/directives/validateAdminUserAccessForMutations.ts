...
export class ValidateAdminUserAccessForMutations extends SchemaDirectiveVisitor {
  public visitFieldDefinition(field: GraphQLField<any, any>) {
    ...
    field.resolve = async (
      ...
      const { hasAdminToken, hasValidAdminToken } = await validateAdminTokenForMutations(
        context,
      ...
      if (hasValidAdminToken) {
        sendAuthMetric(
          ...
          new AuthMetric(
            ...
            metricFields,
            'ValidateAdminUserAccessForMutationsAudit'
          )
        )
      ...
      const { hasApiToken, hasValidApiToken } = await validateApiTokenForMutations(
        context,
        metricFields
      )
      ...
      if (!hasAdminToken && !hasAdminTokenOnHeader && !hasApiToken) {
        logger.warn({
          message: 'ValidateAdminUserAccessForMutations: No token provided',
          ...metricFields,
        })
      ...
      logger.warn({
        message: 'ValidateAdminUserAccessForMutations: Invalid token',
        ...metricFields,
      })
